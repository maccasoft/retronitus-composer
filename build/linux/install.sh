...
# This script adds a menu item, icons and mime type for Retronitus Composer for the
...
RESOURCE_NAME=maccasoft-composer
...
xdg_install_f() {
  ...
  sed -e "s,<BINARY_LOCATION>,${SCRIPT_PATH}/composer,g" \
...
simple_install_f() {
  ...
  # Create *.desktop file using the existing template file
  sed -e "s,<BINARY_LOCATION>,${SCRIPT_PATH}/composer,g" \
      -e "s,<ICON_NAME>,${SCRIPT_PATH}/lib/app.png,g" "${SCRIPT_PATH}/lib/desktop.template" > "${TMP_DIR}/${RESOURCE_NAME}.desktop"
...
display_help_f() {
  printf "\nThis script will add a Retronitus Composer desktop shortcut, menu item,\n"
...
if xdg_exists_f; then
  if [ ${UNINSTALL} = true ]; then
    printf "Removing desktop shortcut and menu item for Retronitus Composer..."
    xdg_uninstall_f
    simple_uninstall_f
  else
    printf "Adding desktop shortcut, menu item and file associations for Retronitus Composer..."
    xdg_uninstall_f
    simple_uninstall_f
    xdg_install_f
  fi
  ...
  if [ ${UNINSTALL} = true ]; then
    ...
    printf "Adding desktop shortcut and menu item for Retronitus Composer..."
    simple_uninstall_f
    simple_install_f
  fi
fi
